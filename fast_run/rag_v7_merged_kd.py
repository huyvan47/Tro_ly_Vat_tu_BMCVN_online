...
DEBUG_RERANK = False
DIRECT_CONF_MIN = 0.55     # conf đủ cao để trả DIRECT_DOC (tinh chỉnh theo KB)
STRICT_CONF_MIN = 0.25     # conf đủ để vào RAG_STRICT
DIRECT_GAP_MIN  = 0.08     # yêu cầu winner rõ ràng theo fused gap
FRAG_GAP_MAX    = 0.03     # gap nhỏ => phân tán => RAG_SOFT (giữ logic cũ)
...
def decide_strategy(norm_query: str, hits: list, filtered_for_main: list) -> str:
    ...
    Dựa trên fused_score + conf (kết hợp rerank_score và embedding score).
    """
    prof = analyze_hits_fused(hits)
    top1, gap, conf = prof["top1"], prof["gap"], prof["conf"]

    has_code = bool(extract_codes_from_query(norm_query))

    # Boost DIRECT nếu query có mã, nhưng vẫn cần winner tương đối rõ
    if CODE_BOOST_DIRECT and has_code and top1 >= 0.55 and gap >= 0.05 and len(filtered_for_main) > 0:
        return "DIRECT_DOC"

    # DIRECT: tự tin cao và winner rõ
    if conf >= DIRECT_CONF_MIN and gap >= DIRECT_GAP_MIN and len(filtered_for_main) > 0:
        return "DIRECT_DOC"

    # Phân tán: gap quá nhỏ -> SOFT
    if gap <= FRAG_GAP_MAX:
        return "RAG_SOFT"

    # Còn lại: nếu conf đủ thì STRICT, không đủ thì SOFT
    if conf >= STRICT_CONF_MIN:
        return "RAG_STRICT"
...
def answer_with_suggestions(user_query: str):
    ...
    filtered_for_main = [h for h in hits if h["fused_score"] >= MIN_SCORE_MAIN]
